...
output_path = 'data/extracted_chords'
...
for midi_file in midi_files:
    try:
        ...
        tonality, chord_list = score.to_romantext_chord_list(tonality=None)
        formatted_chord_list = build_formatted_chord_list(score, chord_list, tonality)
        filepath = midi_file.replace('.mid', '.rntxt').replace('midi_files', 'extracted_chords')
        # Create dir if not exists
        if not os.path.exists(os.path.dirname(filepath)):
            os.makedirs(os.path.dirname(filepath))
        with open(filepath, 'w') as f:
            f.write(formatted_chord_list)
